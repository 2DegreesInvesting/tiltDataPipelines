...
def generate_table(table_name: str) -> None:
    ...
    if table_name == "companies_europages_raw":
    ...
    elif table_name == "companies_companyinfo_raw":
        ...
        country_landingzone = CustomDF("country_landingzone", spark_generate)
        ...
        sources_mapper_raw = CustomDF(
            "sources_mapper_raw",
            spark_generate,
            initial_df=sources_mapper_landingzone.data,
        )
        ...
        countries_mapper_landingzone.data = (
            countries_mapper_landingzone.data.withColumn(
                "country_un",
                F.when(
                    countries_mapper_landingzone.data["country"] == "Namibia", "NA"
                ).otherwise(countries_mapper_landingzone.data["country_un"]),
            )
        )
        ...
        countries_mapper_raw = CustomDF(
            ...
            spark_generate,
            initial_df=countries_mapper_landingzone.data,
        )
        ...
        geography_ecoinvent_mapper_raw = CustomDF(
            ...
            spark_generate,
            initial_df=geography_mapper_landingzone.data,
        )
        ...
        EP_tilt_sector_unmatched_mapper_raw = CustomDF(
            ...
            spark_generate,
            initial_df=EP_tilt_sector_mapper_landingzone.data,
        )
        ...
        tilt_sector_isic_mapper_raw = CustomDF(
            ...
            spark_generate,
            initial_df=tilt_isic_mapper_landingzone.data,
        )
        ...
        tilt_sector_scenario_mapper_raw = CustomDF(
            ...
            spark_generate,
            initial_df=tilt_scenario_mapper_landingzone.data,
        )
    ...
    #     geographies_related.write_table()
        ...
        consequential_ao_raw = CustomDF(
            ...
            spark_generate,
            initial_df=consequential_ao_landingzone.data,
        )
        ...
        impact_categories_raw = CustomDF(
            ...
            spark_generate,
            initial_df=impact_categories_landingzone.data,
        )
        ...
        intermediate_exchanges_raw = CustomDF(
            ...
            spark_generate,
            initial_df=intermediate_exchanges_landingzone.data,
        )
        ...
        elementary_exchanges_raw = CustomDF(
            ...
            spark_generate,
            initial_df=elementary_exchanges_landingzone.data,
        )
        ...
        ecoinvent_co2_raw = CustomDF(
            "ecoinvent_co2_raw",
            spark_generate,
            initial_df=ecoinvent_co2_landingzone.data,
        )
        ...
        ep_ei_matcher_raw = CustomDF(
            "ep_ei_matcher_raw",
            spark_generate,
            initial_df=ep_ei_matcher_landingzone.data,
        )
        ...
        emission_profile_company_landingzone.data = (
            emission_profile_company_landingzone.data.drop("batch")
        )

        emission_profile_company_landingzone.data = (
            emission_profile_company_landingzone.data.distinct()
        )

        emission_profile_company_raw = CustomDF(
            "emission_profile_company_raw",
            spark_generate,
            initial_df=emission_profile_company_landingzone.data,
        )
        ...
        emission_profile_product_landingzone.data = (
            emission_profile_product_landingzone.data.withColumn(
                "multi_match",
                F.when(F.col("multi_match") == "TRUE", F.lit(True))
        ...
        emission_profile_product_landingzone.data = (
            emission_profile_product_landingzone.data.drop("batch")
        )

        emission_profile_product_landingzone.data = (
            emission_profile_product_landingzone.data.distinct()
        )

        emission_profile_product_raw = CustomDF(
            "emission_profile_product_raw",
            spark_generate,
            initial_df=emission_profile_product_landingzone.data,
        )
        ...
        emission_upstream_profile_company_landingzone.data = (
            emission_upstream_profile_company_landingzone.data.drop("batch")
        )
        ...
        emission_upstream_profile_company_landingzone.data = (
            emission_upstream_profile_company_landingzone.data.withColumn(
                "emission_upstream_profile", F.col("emission_usptream_profile")
            ).drop("emission_usptream_profile")
        )
        emission_upstream_profile_company_landingzone.data = (
            emission_upstream_profile_company_landingzone.data.withColumn(
                "emission_upstream_profile_share",
                F.col("emission_usptream_profile_share"),
            )
            .drop("emission_usptream_profile_share")
            .select(col_order)
        )

        emission_upstream_profile_company_landingzone.data = (
            emission_upstream_profile_company_landingzone.data.distinct()
        )

        emission_upstream_profile_company_raw = CustomDF(
            "emission_upstream_profile_company_raw",
            spark_generate,
            initial_df=emission_upstream_profile_company_landingzone.data,
        )
        ...
        emission_upstream_profile_product_landingzone.data = (
            emission_upstream_profile_product_landingzone.data.withColumn(
                "multi_match",
                F.when(F.col("multi_match") == "TRUE", F.lit(True))
                .when(F.col("multi_match") == "FALSE", F.lit(False))
                .otherwise(F.lit(None)),
        ...
        emission_upstream_profile_product_landingzone.data = (
            emission_upstream_profile_product_landingzone.data.drop("batch")
        )
        ...
        emission_upstream_profile_product_landingzone.data = (
            emission_upstream_profile_product_landingzone.data.distinct()
        )

        emission_upstream_profile_product_raw = CustomDF(
            "emission_upstream_profile_product_raw",
            spark_generate,
            initial_df=emission_upstream_profile_product_landingzone.data,
        )
        ...
        sector_profile_company_landingzone.data = (
            sector_profile_company_landingzone.data.distinct()
        )

        sector_profile_company_raw = CustomDF(
            "sector_profile_company_raw",
            spark_generate,
            initial_df=sector_profile_company_landingzone.data,
        )
        ...
        sector_profile_product_landingzone.data = (
            sector_profile_product_landingzone.data.drop("batch")
        )

        sector_profile_product_landingzone.data = (
            sector_profile_product_landingzone.data.distinct()
        )

        sector_profile_product_raw = CustomDF(
            "sector_profile_product_raw",
            spark_generate,
            initial_df=sector_profile_product_landingzone.data,
        )
        ...
        sector_upstream_profile_company_landingzone.data = (
            sector_upstream_profile_company_landingzone.data.drop("batch")
        )

        sector_upstream_profile_company_landingzone.data = (
            sector_upstream_profile_company_landingzone.data.distinct()
        )

        sector_upstream_profile_company_raw = CustomDF(
            "sector_upstream_profile_company_raw",
            spark_generate,
            initial_df=sector_upstream_profile_company_landingzone.data,
        )
        ...
        sector_upstream_profile_product_landingzone.data = (
            sector_upstream_profile_product_landingzone.data.withColumn(
                "multi_match",
                F.when(F.col("multi_match") == "TRUE", F.lit(True))
                .when(F.col("multi_match") == "FALSE", F.lit(False))
                .otherwise(F.lit(None)),
        ...
        sector_upstream_profile_product_landingzone.data = (
            sector_upstream_profile_product_landingzone.data.drop("batch")
        )

        sector_upstream_profile_product_landingzone.data = (
            sector_upstream_profile_product_landingzone.data.distinct()
        )

        sector_upstream_profile_product_raw = CustomDF(
            "sector_upstream_profile_product_raw",
            spark_generate,
            initial_df=sector_upstream_profile_product_landingzone.data,
        )
    ...
    elif table_name == "tiltledger_embedding_raw":

        tiltledger_embedding_landingzone = CustomDF(
            'tiltledger_embedding_landingzone', spark_generate)

        tiltledger_embedding_landingzone.data = tiltledger_embedding_landingzone.data.withColumn(
            'embedding', F.regexp_replace(F.col('embedding'), r'\[|\]', ''))
        tiltledger_embedding_landingzone.data = tiltledger_embedding_landingzone.data.withColumn(
            "embedding", F.split(tiltledger_embedding_landingzone.data["embedding"], ","))
        tiltledger_embedding_landingzone.data = tiltledger_embedding_landingzone.data.withColumn(
            "embedding", F.transform("embedding", lambda x: x.cast("double")))

        tiltledger_embedding_raw = CustomDF(
            'tiltledger_embedding_raw', spark_generate, initial_df=tiltledger_embedding_landingzone.data)
        tiltledger_embedding_raw.write_table()

    elif table_name == 'companies_embedding_raw':

        companies_embedding_landingzone = CustomDF(
            'companies_embedding_landingzone', spark_generate)

        companies_embedding_landingzone.data = companies_embedding_landingzone.data.withColumn(
            'embedding', F.regexp_replace(F.col('embedding'), r'\[|\]', ''))
        companies_embedding_landingzone.data = companies_embedding_landingzone.data.withColumn(
            "embedding", F.split(companies_embedding_landingzone.data["embedding"], ","))
        companies_embedding_landingzone.data = companies_embedding_landingzone.data.withColumn(
            "embedding", F.transform("embedding", lambda x: x.cast("double")))

        companies_embedding_raw = CustomDF(
            'companies_embedding_raw', spark_generate, initial_df=companies_embedding_landingzone.data)
        companies_embedding_raw.write_table()
