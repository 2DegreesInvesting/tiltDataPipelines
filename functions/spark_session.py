...
def create_spark_session() -> SparkSession:
    ...
    if "DATABRICKS_RUNTIME_VERSION" in os.environ:
        ...
        os.environ["OPENAI_API_KEY"] = settings["openai"]["OPENAI_API_KEY"]
        os.environ["OPENAI_EMBEDDING_DIM"] = settings["openai"]["OPENAI_EMBEDDING_DIM"]
        os.environ["OPENAI_MODEL"] = settings["openai"]["OPENAI_MODEL"]

        databricks_settings = settings["databricks"]
        # Build the remote SparkSession using Databricks settings
        spark_session = (
            SparkSession.builder.appName("remote-spark-session")
            .remote(
