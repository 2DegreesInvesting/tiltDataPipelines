...
class CustomDF:
    ...
    def read_table(self):
        ...
        if self._history not in ['recent','complete']:
            raise ValueError(f"Value {self._history} is not in valid arguments [recent,complete] for history argument")
        ...
        try:
            df = read_functions.get(self._schema['type'], read_functions['default'])()
            df.head()  # Force to load first record of the data to check if it throws an error
        except Exception as e:
            if "Path does not exist:" in str(e):
                df = self._spark_session.createDataFrame([], self._schema['columns'])  # If the table does not exist yet, return an empty data frame
            else:
    ...
    def compare_tables(self):
        ...
        value_columns = [F.col(col_name) for col_name in self._df.columns if col_name not in ['tiltRecordID','from_date','to_date']]

        # Read the already existing table
        old_df = CustomDF(self._name, self._spark_session, None, self._partition_name, 'complete')
        old_closed_records = old_df.data.filter(F.col('to_date')!=future_date).select(value_columns + from_to_list)
        old_df = old_df.data.filter(F.col('to_date')==future_date).select(value_columns + from_to_list)
        ...
        new_data_frame = create_sha_values(self._spark_session, self._df, value_columns)
    ...
    def validate_table_format(self):
        ...
        if self._schema['partition_column']:
            check_df = self._spark_session.createDataFrame([], self._schema['columns'].add(self._schema['partition_column'], T.StringType(), False))
        else:
        ...
        try:
            ...
            raise ValueError("The initial structure can not be joined, because: " + str(e)) from e
    ...
    def add_record_id(self) -> DataFrame:
        ...
        data_frame = create_sha_values(self._spark_session, data_frame, sha_columns)
        ...
        if self._partition_name:
            col_order = [x for x in data_frame.columns if x not in ['tiltRecordID', self._partition_name]] + ['tiltRecordID',self._partition_name]
        else:
    ...
    def create_catalog_table(self) -> bool:
        ...
        table_path = build_table_path(self._schema['container'], self._schema['location'],None)
    ...
    def check_signalling_issues(self):
        ...
        dummy_signalling_df = CustomDF('dummy_quality_check',self._spark_session)
        ...
        monitoring_values_df = calculate_signalling_issues(self._spark_session, self._df, signalling_checks, dummy_signalling_df.data)
        monitoring_values_df = monitoring_values_df.withColumn('table_name',F.lit(self._name))

        existing_monitoring_df = CustomDF('monitoring_values',self._spark_session,partition_name=self._name, history='complete')
        ...
        complete_monitoring_partition_df = CustomDF('monitoring_values',self._spark_session,partition_name=self._name,initial_df=monitoring_values_df)
    ...
    def write_table(self):
        ...
        if table_check:
            table_location = build_table_path(self._schema['container'], self._schema['location'], None)
            if partition_column:
                if self._schema['type'] == 'csv':
                    self._df.write.partitionBy(partition_column).mode('overwrite').csv(table_location)
                else:
                    self._df.write.partitionBy(partition_column).mode('overwrite').parquet(table_location)
                
            else:
                if self._schema['type'] == 'csv':
                    self._df.coalesce(1).write.mode('overwrite').csv(table_location)
                else:
                    self._df.coalesce(1).write.mode('overwrite').parquet(table_location)
        else:
    ...
    def custom_join(self, custom_other: 'CustomDF', custom_on: str = None, custom_how: str = None):
        ...
        self._df = self._df.withColumn(
            f'map_{self._name}', F.map_concat(F.col(f'map_{self._name}'), F.col(f'map_{custom_other.name}'))
        )
