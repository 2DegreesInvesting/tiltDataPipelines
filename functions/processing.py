...
def generate_table(table_name: str) -> None:
    ...
    if table_name == 'geographies_raw':
        ...
        geographies_df = df.select(
            'ID', 'Name', 'Shortname', 'Geographical Classification')

        geographies_related_df = df.select(
        ...
        geographies_related_df = geographies_related_df.select(
            'Shortname', 'Shortname_related')

        write_table(spark_generate, geographies_df, 'geographies_transform')

        write_table(spark_generate, geographies_related_df,
                    'geographies_related')
