...
class PreProcessor():
    ...
    def preprocess(self, spark_session, companies_europages_raw):
        print(spark_session)
        ...
        translated_df = translated_df.withColumn('new_product_id', F.udf(self.create_new_id, StringType())("product_name"))
        ...
        print("Deduplicating...")
        # then deduplicate
        deduplicated_products, companies_products_mapping = self.deduplicate_df(spark_session, translated_df, companies_products_raw)
        
        print("Preprocessing finished.")
        return deduplicated_products, companies_products_mapping
    
    def create_new_id(self, text):
    ...
    def typo_correct_df(self, df):
        ...
        to_process_df = to_process_df.withColumn("language (ISO-code)", F.udf(self.conf_ld_detect_language, StringType())("product_name"))
        ...
        english_df = english_df.withColumn("typo_corrected", F.udf(self.typo_correction, StringType())("product_name"))
    ...
    def translate_df(self, df):
        ...
        non_english_df = non_english_df.withColumn("translated_text", F.udf(self.translate_Google, StringType())("typo_corrected"))
    ...
    def deduplicate_df(self, spark_session, translated_df, companies_products_mapping):
